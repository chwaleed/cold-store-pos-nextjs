...
const DELAY_AFTER_SIMPLE_BATCH = 5000; // 5 seconds

// For complex loops (Receipts)
const COMPLEX_LOOP_BATCH_SIZE = 1000;
const DELAY_AFTER_COMPLEX_BATCH = 5000; // 5 seconds
...
async function main() {
  ...
  // Converted to a sequential loop to avoid SQLite concurrency panics
  const productTypes = [];
  for (let i = 0; i < productTypeNames.length; i++) {
    const name = productTypeNames[i];
    const productType = await prisma.productType.upsert({
      where: { name },
      update: {},
      create: {
        name,
        doubleRentAfter30Days: i % 3 === 0,
      },
    });
  ...
  for (let i = 0; i < subTypeNames.length; i++) {
    ...
    // *** CHANGED TO UPSERT ***
    // This fixes the P2002 Unique Constraint error by making
    // this part of the script idempotent (safe to run multiple times).
  ...
  for (const name of packNames) {
    const pack = await prisma.packType.upsert({
      where: { name },
      update: {},
      create: { name },
    });
  ...
  const customers = [];
  const customerBatchSize = 100; // This batch size is fine, 1000 is small
  ...
  for (let i = 0; i < 1000; i += customerBatchSize) {
    ...
    await delay(50); // Keeping this small, 1000 customers is fast
  }
  ...
  // Using sequential upsert for idempotency
  for (let i = 0; i < 100; i++) {
    const prefix = categoryPrefixes[i % categoryPrefixes.length];
    const name = `${prefix} ${Math.floor(i / categoryPrefixes.length) + 1}`;
  ...
  console.log(
    `  ℹ️ This is a complex loop. Will pause for ${
      DELAY_AFTER_COMPLEX_BATCH / 1000
    }s every ${COMPLEX_LOOP_BATCH_SIZE} receipts.`
  );
